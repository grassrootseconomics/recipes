...
Every recipe requires exactly 6 total voucher items, decoupled from the 8 fixed voucher cards per ingredient. Two vouchers from each active participant start in the central platter, leaving 6 own cards in hand for play. Voucher cards are reusable claims on a finite real ingredient stock. The MVP uses a fixed stock of 40 real units per active ingredient owner, enough for full 3-dish catalog demand plus backing for the 8 voucher cards.
...
At the start of the game, every active participant automatically contributes exactly two active vouchers into the central platter pool. The client should show this as the opening offering animation, not as a manual per-player button flow. The central platter is visible to everyone.
...
Players can also trade directly with each other. A player zooms out to see the full table, clicks another player, and creates a structured exchange offer. The offer panel shows the offered and requested asset cards, a grouped public summary of the other player's current hand assets, and a public recipe-help summary for the other player: their current recipe name and the ingredient counts still missing after counting recipe progress and useful promise cards already in that player’s hand. Clicking a visible item in the other player's hand can ask for that exact promise-card group or dish-piece group. There is no free chat in the MVP. Offers can be accepted, refused, or cancelled. Assets in pending offers are locked until the offer resolves.
...
Hosted online tables are public by default while they are still joinable in the lobby. The host can switch a lobby table between public and private. Public joinable tables are listed by the server so online clients can browse and select them without typing an invite code. Private tables remain joinable by invite code while they still have bot seats available, but they are not advertised in the public table list. Started tables, full tables, and closed/completed tables must not appear in the public list.
...
Bots see only legal player information: their own hand, their own recipe, grouped public hand asset summaries for other players, public current-recipe help summaries for other players, the visible central platter, public dish counts, and offers involving them. Bots cannot inspect individual hidden voucher IDs or full hidden recipe state.
...
End-game statistics use explicit terms:
...
- 8 fixed vouchers per ingredient.
...
- public lobby tables are listed by the server only while joinable.
- private lobby tables remain joinable by invite code but are omitted from public listing.
- started or full tables are omitted from public listing.
...
- total vouchers per ingredient owner always equals 8.
...
- full catalog demand plus 8 voucher-backing units for the fixed 3-dish goal never exceeds per-owner real stock.
...
- the fixed dish goal is 3.
- platter deposit and 1:1 swaps are atomic.
- structured offers lock cards and resolve correctly.
- offer popups show ingredient cards, grouped public hand assets for direct negotiation, and public current-recipe help summaries without exposing individual hidden card IDs or full hidden recipe state.
...
- round-robin turns enforce the clockwise active-seat order and advance deterministically.
- active players can see grouped hand assets in offer popups, but cannot see individual voucher IDs or full hidden recipe state.
